...
현재 staged 코드가 이미 이 영역을 직접 보강하고 있다.
- `PUBLISH` 초기 body 기본값
...
## IMS private / carrier-specific surface
| 영역 | 이유 | 필요한 타깃 |
| --- | --- | --- |
...
이 영역은 Mutator가 이미 만드는 wire/byte artifact를 `fuzzer send packet`으로 직접 흘릴 수 있을 때 본격적으로 실험한다.
...
- sender/reactor가 핵심 메서드 baseline packet을 보낼 수 있어야 한다.
